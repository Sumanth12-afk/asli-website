...
import { OrbitControls, useGLTF, useFBX, useProgress, Html, Environment, ContactShadows, useTexture } from '@react-three/drei';
    ...
    return (
        <Html center>
            {placeholderSrc ? (
                ...
                <div className="text-white bg-black/50 px-2 py-1 rounded">{Math.round(progress)} %</div>
            )}
        </Html>
    );
};
    ...
    return (
        <group ref={outer}>
            <group ref={inner}>
                {isImage ?
                    <ImagePlane url={url} /> :
                    (content && <primitive object={content} />)
                }
            </group>
        </group>
    );
...
const ModelViewer = ({
    ...
    const cleanUrl = url && url.startsWith('http') ? url : (url || '');
